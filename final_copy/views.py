...
def final_copy(request):
    ...
    try:
        # Download all input files to temp_dir
        for url in files:
            try:
        ...
        # Upload and convert all files to PDFs via PDF.co
        pdf_urls = []
        for file_path in temp_paths:
            try:
                file_name, ext = os.path.splitext(os.path.basename(file_path))
                uploaded_url = upload_to_pdfco(file_path)
                pdf_url = convert_to_pdf(uploaded_url, ext, file_name)
        ...
        # erge all PDF URLs via PDF.co
        output_pdf = os.path.join(temp_dir, "final_copy.pdf")
        merge_pdfs(pdf_urls, output_pdf)

        # Move merged file to MEDIA_ROOT/generated/
