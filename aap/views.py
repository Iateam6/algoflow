...
def index(request):
    return HttpResponse('Welcome to Application for Advance Parole / Travel Authorization (I-131) API!')
...
def generate_doc(request):
    ...
    try:
        ...
        logger.info("Downloaded %s files for AAP generation", len(file_manifests))
        ...
        logger.exception("Failed to download one or more AAP source documents.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        logger.exception("Unexpected error while generating an AAP document.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        try:
            ...
            logger.warning("Failed to clean temporary AAP directory %s", temp_dir)
