...
def index(request):
    return HttpResponse('Welcome to EB-1A I-140 & I-485 with Sponsors API!')
...
def generate_doc(request):
    ...
    try:
        ...
        logger.info("Downloaded %s files for EB-1aB generation", len(file_manifests))
        ...
        logger.exception("Failed to download one or more EB-1aB source documents.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        logger.exception("Unexpected error while generating an EB-1aB document.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        try:
            ...
            logger.warning("Failed to clean temporary eb-1aB directory %s", temp_dir)
