...
def generate_doc(request):
    ...
    try:
        ...
        logger.info("Downloaded %s files for AEA generation", len(file_manifests))
        ...
        logger.exception("Failed to download one or more AEA source documents.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        logger.exception("Unexpected error while generating an AEA document.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        try:
            ...
            logger.warning("Failed to clean temporary AEA directory %s", temp_dir)
