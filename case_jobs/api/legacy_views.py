...
def legacy_index(request, visa_type: str):
    adapter = get_adapter(visa_type)
    return HttpResponse(f"Welcome to {adapter.display_name} API!")
...
@csrf_exempt
def legacy_generate_doc(request, visa_type: str):
    """
    ...
    try:
        ...
        logger.info(
            "Downloaded %s files for %s generation",
            len(file_manifests),
            adapter.display_name,
        )

        generated_paths = handle_doc_generation(file_manifests, [options], adapter)
        ...
        logger.exception(
            "Failed to download one or more %s source documents.",
            adapter.display_name,
        )
        return JsonResponse(
            {
        ...
        logger.exception(
            "Unexpected error while generating a %s document.",
            adapter.display_name,
        )
        return JsonResponse(
            {
        ...
        try:
            ...
            logger.warning(
                "Failed to clean temporary %s directory %s",
                adapter.display_name,
                temp_dir,
            )
