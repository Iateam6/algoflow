...
def generate_doc(request):
    ...
    try:
        ...
        logger.info("Downloaded %s files for DS-260 generation", len(file_manifests))
        ...
        logger.exception("Failed to download one or more DS-260 source documents.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        logger.exception("Unexpected error while generating an DS-260 document.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        try:
            ...
            logger.warning("Failed to clean temporary DS-260 directory %s", temp_dir)
