...
def index(request):
    return HttpResponse('Welcome to EB-1A I-140 with Sponsors API!')
...
def generate_doc(request):
    ...
    try:
        ...
        logger.info("Downloaded %s files for EB-1A generation", len(file_manifests))
        ...
        logger.exception("Failed to download one or more EB-1A source documents.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        logger.exception("Unexpected error while generating an EB-1A document.")
        return JsonResponse(
            {
                "download_url": None,
                "error message": "Document generation failed",
        ...
        try:
            ...
            logger.warning("Failed to clean temporary EB-1A directory %s", temp_dir)
